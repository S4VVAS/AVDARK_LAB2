...
void user_lock_clh::lock(int thread_id) {
    ...
    //make L point to I and P point to L
    l->previous = m_tail.load();
    cell *prev = l->previous;
    while(m_tail.compare_exchange_strong(prev,l->local_cell)){}

    //spin untill other thread releases lock
